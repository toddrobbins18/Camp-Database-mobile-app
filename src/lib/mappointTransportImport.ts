import { parseCSV } from './sunshineCsv';
import bundledGeocodes2026 from '../../assets/transport/north_shore_mappoint_geocodes_2026.json';
import mappointRoutesCsv2026 from './mappointRoutes2026Csv';
...
/** Bundled MapPoint routes CSV (Metro cannot require .csv reliably). */
export async function getBundledMappointRoutesCsv2026(): Promise<string> {
...
export function seedGeocodeCacheFromBundled(cache: Map<string, GeocodeCacheEntry | null>) {
  for (const [key, hit] of Object.entries(bundledGeocodeMap)) {
    if (!cache.has(key)) {
      cache.set(key, { lat: hit.lat, lng: hit.lng, provider: hit.provider || 'ors' });
    }
  }
}
...
export function parseMappointRoutesCsv(
  ...
  const rows = parseCSV(csvText) as unknown as MappointRouteRow[];
  ...
  for (const row of rows) {
    ...
    if (!route.busCounselor && row.bus_counselor) route.busCounselor = row.bus_counselor.trim();
