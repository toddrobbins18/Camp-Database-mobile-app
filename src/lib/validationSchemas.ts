...
// Menu items validation schema (meal_type normalized to lowercase in parser)
...
// Sports Academy (enrollment) — matches CSVFormatGuide sports_academy
export const sportsAcademySchema = z.object({
    person_id: z.string().trim().min(1, 'Person ID is required'),
    sport_name: z.string().trim().min(1, 'Sport is required'),
    instructor: z.string().optional().nullable(),
    start_date: z.string().optional().nullable(),
    end_date: z.string().optional().nullable(),
    notes: z.string().optional().nullable(),
    schedule_periods: z.array(z.string()).optional(),
});

// Special events & evening activities
export const specialEventActivitySchema = z.object({
    title: z.string().min(1, 'Title is required'),
    ...
    chaperone: z.string().optional().nullable(),
...
export function parseChildRow(row: Record<string, any>) {
    ...
    return {
        ...
        age: row.age || row.Age ? parseInt(String(row.age || row.Age), 10) : null,
...
export function parseIncidentReportRow(row: Record<string, any>) {
    const personIdString = String(
        row.person_id || row['Person ID'] || row.PersonID || row.person_ids || row['Person IDs'] || ''
    ).trim();
...
export function parseSportsAcademyRow(row: Record<string, any>) {
    const sport = row.sport || row.sport_name || row.Sport || '';
    const sessionRaw = row.session || row.schedule_periods || row['Schedule Periods'] || '';
    const periods = sessionRaw
        ? String(sessionRaw)
              .split(',')
              .map((s) => s.trim())
              .filter(Boolean)
        : undefined;
    return {
        person_id: String(row.person_id || row['Person ID'] || row.PersonID || '').trim(),
        sport_name: String(sport).trim(),
        instructor: row.instructor ? String(row.instructor) : null,
        start_date: row.enrollment_date || row.start_date ? String(row.enrollment_date || row.start_date) : null,
...
export function parseSpecialEventActivityRow(row: Record<string, any>) {
    return {
        title: String(row.title || row.Title || ''),
        event_type: String(row.event_type || row['Event Type'] || row.eventType || ''),
        event_date: String(row.event_date || row['Event Date'] || row.date || ''),
        time_slot: String(row.time_slot || row['Time Slot'] || row.timeSlot || ''),
        start_time: String(row.start_time || row['Start Time'] || ''),
        end_time: row.end_time || row['End Time'] ? String(row.end_time || row['End Time']) : null,
        ...
        chaperone: row.chaperone != null && row.chaperone !== '' ? String(row.chaperone) : null,
        division_id: row.division_id ? String(row.division_id).trim() : null,
    };
}
