...
export function SunshineReportScreen({ navigation }: any) {
  ...
  useEffect(() => {
    if (companyId) {
      void fetchData({ syncRoster: true });
    }
  }, [companyId, season]);
  ...
  const fetchData = async (options?: { syncRoster?: boolean }) => {
    if (!companyId) return;
    setLoading(true);
    try {
      if (options?.syncRoster) {
        ...
        try {
          await syncSunshineFromRoster(companyId, season);
        } catch (e: any) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.actionsBar}>
        <TouchableOpacity
          ...
          onPress={() => void fetchData({ syncRoster: true })}
        >
          <Ionicons name="refresh-outline" size={16} color={theme.colors.primary} />
          <Text style={styles.actionButtonOutlineText}>{syncingRoster ? 'Loading…' : 'Reload Roster'}</Text>
        </TouchableOpacity>
