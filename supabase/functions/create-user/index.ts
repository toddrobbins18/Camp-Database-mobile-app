...
serve(async (req) => {
  ...
  try {
    ...
    // Check if user is admin or super_admin (must have a row in user_roles)
    ...
    if (!targetCompanyId) {
      return jsonResponse(
        { error: 'No company selected. Select a camp from the app menu first, or ensure your profile has a company assigned.' },
        400
      );
    }
    ...
    // Ensure profile exists and is approved (trigger may create it; we set approved + company so they can log in)
    await supabaseAdmin
      .from('profiles')
      .upsert({
        id: newUser.user.id,
    ...
    if (roleError) {
      ...
      return jsonResponse(
        { error: `Role assignment failed: ${roleError.message}. Check that the role is valid (e.g. staff, admin, viewer).` },
        400
      );
    }
